...
def query_and_preserve(headers):
    ...
    if high_speed:
        ...
        trip_ids = _query_normal_ticket(
            place_pair=other_place_pair, headers=headers, time=date
        )
    ...
    res = requests.post(url=PRESERVE_URL, headers=headers, json=base_preserve_payload)
